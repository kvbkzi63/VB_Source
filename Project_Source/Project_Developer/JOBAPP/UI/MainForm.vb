...
Public Class MainForm
    ...
    Private Function MenuFindDocument(ByVal text As String, Optional ByVal header As String = "") As IDockContent
        Try
            If dockPanel.DocumentStyle = DocumentStyle.SystemMdi Then
            ...
            Else
                For Each content As IDockContent In dockPanel.Contents '.Documents
                    If content.GetType().ToString = text AndAlso content.ToString.Contains(header) Then
                        Return content
                    End If
                Next
                Return Nothing
    ...
    Private Sub setBlackColor()
        Try
            ...
            If (MyDialog.ShowDialog() = Forms.DialogResult.OK) Then
    ...
    Private Sub setFont()
        ...
        If (fontDiaglog.ShowDialog() = Forms.DialogResult.OK) Then
...
rep:        Using loginForm As New LoginForm()
    ...
    Public Function DisplayCurrentVersion() As String
        ...
        Try
            ...
            CurrentVersion = "版本:V" & GetLocalVer.ToString
...
#Region "Menu相關動作"
    ...
    Private Sub windowNewMenu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
            ...
            Dim isDockShow As DockContent = CType(MenuFindDocument(tagStrings(1), item), Com.Syscom.WinFormsUI.Docking.DockContent)
            If isDockShow IsNot Nothing AndAlso tagStrings(0) <> "Dummy.dll" AndAlso isDockShow.Text = item Then
                isDockShow.Show()
            ...
            If dockPanel.DocumentStyle = DocumentStyle.SystemMdi Then
                dockForm.MdiParent = JOBAPP.MainForm.ActiveForm
                dockForm.Show()
            Else
                Me.Cursor = System.Windows.Forms.Cursors.WaitCursor
            ...
            End If
            '拋出報表名稱與ID
            eventManager.RaiseReportIdEvent(dockForm.Tag, item)
...
#Region "add on 2012-09-18 by Lits for 我的最愛"
    ...
    Private Sub MyFavorites()
        Dim MyFavoritesUI As MyFavoritesUI = New MyFavoritesUI
...
#Region "Get Local Version"
    Private Function GetLocalVer() As Int32

        Dim Ver As Int32 = 1
        Dim VerUpdateDir As New DirectoryInfo(System.Windows.Forms.Application.StartupPath & "\VerUpdate")
        Try

            If VerUpdateDir.Exists AndAlso VerUpdateDir.GetDirectories.Length > 0 Then
                Return VerUpdateDir.GetDirectories.Length
            Else
                Return 1
            End If
