...
'*****************************************************************************
'*
'*    Page/Class Name:	PUBPartUI.vb
'*              Title:	部份負擔基本檔維護
'*        Description:	主要對部份負擔基本檔做確定儲存和查詢操作
...
Public Class PUBPartUI
    ...
    '獲取維護表名
    Dim tableDB As String = PubPartDataTableFactory.tableName
    'Grid使用的標題
    Dim columnNameGrid() As String = {"生效日", "部份負擔代碼", "名稱", "負擔金額", "負擔比率", "結束日", "停用", "建立者", "建立時間", "修改者", "修改時間", "生效日1", "結束日1"}
    '獲取維護表字段名
    Dim columnNameDB() As String = PubPartDataTableFactory.columnsName
    '獲取維護表字段長度
    Dim columnsLength() As Integer = PubPartDataTableFactory.columnsLength
    '資料集類型定義
    Enum DataSet_Type
        Grid = 0
        DB = 1
    ...
    Public Sub initialData()
        Try
            ...
            '設定欄位長度
            Me.txtPartCode.MaxLength = columnsLength(1)
            Me.txtPartName.MaxLength = columnsLength(2)
    ...
    Public Function queryData() As Boolean
        ...
        Try
            ...
            End If
            '執行查詢
            dsDB = objPub.queryPUBPartByCond(dateEffectDate, txtPartCode.Text.Trim)
    ...
    Public Function confirmPUBPart() As Boolean
        ...
        Try
            ...
            drDB("Part_Code") = Me.txtPartCode.Text.Trim
            drDB("Part_Name") = Me.txtPartName.Text.Trim.Replace("'", "''")
            drDB("Part_Amt") = Me.txtPartAmt.Text.Trim
            drDB("Part_Ratio") = Me.txtPartRatio.Text.Trim
            ...
            '確定儲存健保部份負擔資料
            dsReturn = objPub.confirmPUBPart(dsDB)
    ...
    Private Function fieldCheckResult(ByVal iButtonFlag As Integer) As Boolean
        Try
            ...
            Dim ctlTxtPartCode As Control = Me.txtPartCode
            Dim ctlTxtPartName As Control = Me.txtPartName
            Dim ctlTxtPartAmt As Control = Me.txtPartAmt
            Dim ctlTxtPartRatio As Control = Me.txtPartRatio
            ...
            Select Case iButtonFlag
                Case buttonAction.OK
                    If Not fieldCheckNull(ctlDtpEffectDate, Control_Type.DateTimePicker) Then
                        ...
                        blnReturnFlag = True
                    End If
                    If Not fieldCheckNull(ctlTxtPartCode, Control_Type.TextBox) Then
                        If (strErrMsg1.ToString.Trim.Length > 0) Then strErrMsg1.Append(",")
                        strErrMsg1.Append("部份負擔代碼")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartCode
                        End If
                        blnReturnFlag = True
                    End If
                    If Not fieldCheckNull(ctlTxtPartName, Control_Type.TextBox) Then
                        If (strErrMsg1.ToString.Trim.Length > 0) Then strErrMsg1.Append(",")
                        strErrMsg1.Append("名稱")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartName
                        End If
                        blnReturnFlag = True
                    End If
                    If Not fieldCheckNull(ctlTxtPartAmt, Control_Type.TextBox) Then
                        If (strErrMsg1.ToString.Trim.Length > 0) Then strErrMsg1.Append(",")
                        strErrMsg1.Append("負擔金額")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartAmt
                        End If
                        blnReturnFlag = True
                    End If
                    If Not checkIsDecimal(ctlTxtPartAmt, 1) Then
                        strErrMsg3.Append("負擔金額")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartAmt
                        End If
                        blnReturnFlag = True
                    End If
                    If Not fieldCheckNull(ctlTxtPartRatio, Control_Type.TextBox) Then
                        If (strErrMsg1.ToString.Trim.Length > 0) Then strErrMsg1.Append(",")
                        strErrMsg1.Append("負擔比率")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartRatio
                        End If
                        blnReturnFlag = True
                    End If
                    If Not checkIsDecimal(ctlTxtPartRatio, 4) Then
                        If (strErrMsg3.ToString.Trim.Length > 0) Then strErrMsg3.Append(",")
                        strErrMsg3.Append("負擔比率")
                        If blnReturnFlag = False Then
                            ctlObjFocus = ctlTxtPartRatio
                        End If
                        blnReturnFlag = True
    ...
    Private Function dbCheck() As Boolean
        ...
        Try
            '先以部份負擔代碼作為條件查詢出所有記錄
            dsDB = objPub.queryPUBPartByCond(System.DateTime.MinValue, txtPartCode.Text.Trim)
    ...
    Private Sub cleanFieldsColor()
        Try
            ...
            Me.txtPartCode.BackColor = ScreenUtil.BACK_COLOR_DEFAULT
            Me.txtPartName.BackColor = ScreenUtil.BACK_COLOR_DEFAULT
            Me.txtPartAmt.BackColor = ScreenUtil.BACK_COLOR_DEFAULT
            Me.txtPartRatio.BackColor = ScreenUtil.BACK_COLOR_DEFAULT
    ...
    Public Sub dgvCellClick()
        Try
            ...
            If dgvShowData.CurrentCellAddress.Y >= 0 Then
                Me.txtPartCode.Text = dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("部份負擔代碼").Value.ToString.Trim
                Me.txtPartName.Text = dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("名稱").Value.ToString.Trim
                Me.txtPartAmt.Text = dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("負擔金額").Value.ToString.Trim
                Me.txtPartRatio.Text = dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("負擔比率").Value.ToString.Trim
                Me.dtpEndDate.SetValue(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("結束日1").Value.ToString)
    ...
    Private Sub bindDataGridView(ByVal ds As DataSet, ByVal iButtonFlag As Integer)
        ...
        For i As Integer = 0 To ds.Tables(tableDB).Rows.Count - 1
            ...
            drGrid(i)("名稱") = ds.Tables(tableDB).Rows(i)("Part_Name").ToString.Trim()
            drGrid(i)("負擔金額") = ds.Tables(tableDB).Rows(i)("Part_Amt").ToString.Trim()
            drGrid(i)("負擔比率") = ds.Tables(tableDB).Rows(i)("Part_Ratio").ToString.Trim()
            drGrid(i)("結束日") = DateUtil.TransDateToROC(CDate(ds.Tables(tableDB).Rows(i)("End_Date")).Year & CDate(ds.Tables(tableDB).Rows(i)("End_Date")).ToString("/MM/dd"))
            drGrid(i)("結束日1") = CDate(ds.Tables(tableDB).Rows(i)("End_Date")).ToString("yyyy/MM/dd")
        ...
        dgvShowData.Columns(11).Visible = False
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        Try
            ...
            End If
        Catch ex As Exception
            'Mark By Willlog.Error(ex.ToString)
