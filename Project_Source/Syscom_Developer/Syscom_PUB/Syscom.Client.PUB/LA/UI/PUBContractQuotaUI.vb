...
'*****************************************************************************
'*
'*    Page/Class Name:	PUBContractQuotaUI.vb
'*              Title:	合約機構記帳累計檔維護
'*        Description:	主要對合約機構記帳累計檔做確定儲存和查詢操作
'*          Copyright:	Copyright(c) Syscom Computer Co,.Ltd
'*            Company:	Syscom Computer Co,.Ltd.
'*            @author:	Yunfei
'*        Create Date:	2009/07/21
'*      Last Modifier:	Yunfei
'*   Last Modify Date:	2009/08/18
'*
'*****************************************************************************
'*/
...
Public Class PUBContractQuotaUI
    ...
    '獲取維護表名
    Dim tableDB As String = PubContractQuotaDataTableFactory.tableName
    'Grid使用的標題
    Dim columnNameGrid() As String = {"生效日", "合約機關ID", "合約機關代碼", "身份二ID", "身份二代碼", "額度", "已用額度", "停止日", "停用", "建立者", "建立時間", "修改者", "修改時間"}
    '獲取維護表字段名
    Dim columnNameDB() As String = PubContractQuotaDataTableFactory.columnsName
    '獲取維護表字段長度
    Dim columnsLength() As Integer = PubContractQuotaDataTableFactory.columnsLength
    '資料集類型定義
    Enum DataSet_Type
        Grid = 0
        DB = 1
    ...
    Public Sub initialData()
        Try
            ...
            dgvShowData.Columns(1).Visible = False
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub initialCmbContractCode()
        Dim dsDB As DataSet
        '執行查詢()
        dsDB = objPub.queryPUBContractByColumnValue((New String() {"Check_Quota_Id", "Sub_Identity_Code", "DC"}), New Object() {2, Me.cmbSubIdentityCode.SelectedValue.ToString.Trim, "N"})
    ...
    Public Function queryData() As Boolean
        ...
        Try
            ...
            End If
            '執行查詢
            dsDB = objPub.queryPUBContractQuotaByCond(dateEffectDate, cmbContractCode.SelectedValue.Trim.Replace("'", "''"), Me.cmbSubIdentityCode.SelectedValue.Trim.Replace("'", "''"))
            If dsDB.Tables.Count > 0 Then
    ...
    Public Function confirmPUBContractQuota() As Boolean
        ...
        Try
            ...
            drDB("Quota_Amt") = Me.txtQuotaAmt.Text.Trim.Replace("'", "''")
            If Me.txtUsedQuotaAmt.Text.Trim <> "" Then
                drDB("Used_Quota_Amt") = Me.txtUsedQuotaAmt.Text.Trim.Replace("'", "''")
            Else
            ...
            '確定儲存合約機構記帳累計資料
          dsReturn = objPub.confirmContractQuota(dsDB)
    ...
    Private Function dbCheck() As Boolean
        ...
        Try
            '先以合約機關ID作為條件查詢出所有記錄
            dsDB = objPub.queryPUBContractQuotaByCond(System.DateTime.MinValue, cmbContractCode.SelectedValue.Trim.Replace("'", "''"), Me.cmbSubIdentityCode.SelectedValue.Trim.Replace("'", "''"))
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub PUBContractQuotaUI_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub PUBContractQuotaUI_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            initialData()
        Catch ex As Exception
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        Try
            ...
            If (confirmPUBContractQuota()) Then
