...
Public Class PUBAreaCodeGovSetupUI
...
#Region "     *Table 欄位 "
    ...
    '存檔的Table欄位
    Private gblColumnNameDB As String() = PubAreaCodeGovDataTableFactory.columnsName

    '存檔的Table Name
    Private gblTableName As String = PubAreaCodeGovDataTableFactory.tableName

    '空的DT，清空資料使用
    Private gblEmptyDT As DataTable = PubAreaCodeGovDataTableFactory.getDataTable

    '要顯示在畫面上的Grid 中文標頭，不顯示的也要有名稱
    Private gblColumnTextDgv As String = "縣市代碼,縣市名稱,區里代碼,區里名稱,村里代碼,村里名稱"

    '要顯示在畫面上的Grid 欄位寬度，不顯示的設0
    Private gblColumnWidthDgv As String = "100,200,100,200,100,200"
...
#Region "     *新增 "
    ...
    Public Overrides Function InsertData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.PUBAreaCodeGovInsertByPK(InsertDS)
            ...
            If count > 0 Then
                ...
                'updateDataGridView(iButtonFlag, PUB.PUBAreaCodeGovQueryByPKROC(txt_GovCountyCode.Text))
                queryDS = PUB.queryAreaCodeGovAll()
                '顯示資料在 Grid 上
...
#Region "     *修改 "
    ...
    Public Overrides Function updateData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.PUBAreaCodeGovUpdateByPK(updateDS)
...
#Region "     *刪除 "
    ...
    Public Overrides Function deleteData() As Boolean
        ...
        Try
            ...
            dsDelete.Tables.Add()
            dsDelete.Tables(0).Columns.Add("Gov_County_Code")
            dsDelete.Tables(0).Columns.Add("Dist_Code")
            dsDelete.Tables(0).Columns.Add("Vil_Code")

            dsDelete.Tables(0).Rows.Add()
            dsDelete.Tables(0).Rows(0).Item("Gov_County_Code") = txt_GovCountyCode.Text
            dsDelete.Tables(0).Rows(0).Item("Dist_Code") = txt_DistCode.Text
            dsDelete.Tables(0).Rows(0).Item("Vil_Code") = txt_VillCode.Text
            ...
            If dgvShowData.GetSelectedIndex IsNot Nothing AndAlso dgvShowData.GetSelectedIndex.Count > 0 Then
                ...
                For i As Integer = 0 To dgvShowData.GetSelectedIndex.Count - 1
                    ...
                    dsDelete.Tables(0).Rows.Add()
                    dsDelete.Tables(0).Rows(count).Item("Area_Code") = drData.Item("Area_Code")
                    dsDelete.Tables(0).Rows(count).Item("Orig_Area_Code") = drData.Item("Orig_Area_Code")
                    count = count + 1
            ...
            '自資料庫刪除
            count = PUB.PUBAreaCodeGovdeleteChoose(dsDelete)
...
#Region "     *查詢 "
    ...
    Public Overrides Function queryData() As Boolean
        ...
        Try
            ...
            If txt_GovCountyCode.Text <> "" Then
                queryDS = PUB.PUBAreaCodeGovQueryByPKROC(txt_GovCountyCode.Text)
            Else
                queryDS = PUB.queryAreaCodeGovAll()
            End If
            '*******************************************************************************
...
#Region "     *欄位檢查 - True 檢查不通過;False 檢查通過 "
    ...
    Public Function fieldCheckResult(ByVal iButtonFlag As Integer) As Boolean
        ...
        Try
            ...
            '設定檢查的控制項、檢查未通過的顯示名稱
            Dim objCheck1 As UCLTextBoxUC = txt_GovCountyCode
            Dim errorStr1 As String = "縣市代碼"

            Dim objCheck2 As UCLTextBoxUC = txt_DistCode
            Dim errorStr2 As String = "區里代碼"

            Dim objCheck3 As UCLTextBoxUC = txt_VillCode
            Dim errorStr3 As String = "村里代碼"
    ...
    Public Overrides Sub dgvCellClick()
        ...
        Try
            ...
            If dgvShowData.CurrentCellAddress.Y >= 0 Then
                ...
                '設定縣市代碼的控制項
                txt_GovCountyCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Gov_County_Code").Value)

                '設定縣市名稱的控制項
                txt_GovCountyName.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Gov_County_Name").Value)

                '設定區里代碼的控制項
                txt_DistCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Dist_Code").Value)

                '設定區里名稱的控制項
                txt_DistName.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Dist_Name").Value)

                '設定村里代碼的控制項
                txt_VillCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Vil_Code").Value)

                '設定村里名稱的控制項
                txt_VillName.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Vil_Name").Value)
...
#Region "     *取得要存檔的Dataset "
    ...
    Private Function GetSaveDS(ByVal inputCreateUser As String, ByVal inputModifiedUser As String) As DataSet
        ...
        Try
            ...
            '{ Station_No,Print_Time,Printer_Code,Create_User,Create_Time,Modified_User ,Modified_Time   }
            drSave("Gov_County_Code") = nvl(txt_GovCountyCode.Text)

            drSave("Gov_County_Name") = nvl(txt_GovCountyName.Text)

            drSave("Dist_Code") = nvl(txt_DistCode.Text)

            drSave("Dist_Name") = nvl(txt_DistName.Text)

            drSave("Vil_Code") = nvl(txt_VillCode.Text)

            drSave("Vil_Name") = nvl(txt_VillName.Text)
