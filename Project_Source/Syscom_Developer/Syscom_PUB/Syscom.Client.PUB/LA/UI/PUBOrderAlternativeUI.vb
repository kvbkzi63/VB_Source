...
Public Class PUBOrderAlternativeUI
...
#Region "     *Table 欄位 "
    ...
    '存檔的Table欄位
    Private gblColumnNameDB As String() = PubOrderAlternativeDataTableFactory.columnsName

    '存檔的Table Name
    Private gblTableName As String = PubOrderAlternativeDataTableFactory.tableName

    '空的DT，清空資料使用
    Private gblEmptyDT As DataTable = PubOrderAlternativeDataTableFactory.getDataTable

    '要顯示在畫面上的Grid 中文標頭，不顯示的也要有名稱
    Private gblColumnTextDgv As String = "醫令代碼,醫令名稱,可替代醫令代碼,替代醫令名稱,建立者,建立時間,修改者,修改時間"

    '要顯示在畫面上的Grid 欄位寬度，不顯示的設0
    Private gblColumnWidthDgv As String = "100,150,100,150,80,120,80,120"
...
#Region "     *新增 "
    ...
    Public Overrides Function InsertData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.insertPUBOrder(InsertDS)
            ...
            If count > 0 Then
                ...
                'updateDataGridView(iButtonFlag, PUB.PUBOrderAlternativequeryByPKOrderCode(txt_OrderCode.Text))
                Dim queryDS As DataSet
                queryDS = PUB.PUBOrderAlternativequeryByPKOrderCode(txt_OrderCode.Text.Trim)
...
#Region "     *修改 "
    ...
    Public Overrides Function updateData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.updatePUBOrder(updateDS)
...
#Region "     *刪除 "
    ...
    Public Overrides Function deleteData() As Boolean
        ...
        Try
            ...
            dsDelete.Tables.Add()
            dsDelete.Tables(0).Columns.Add("Order_Code")
            dsDelete.Tables(0).Columns.Add("Alternative_Code")

            dsDelete.Tables(0).Rows.Add()
            dsDelete.Tables(0).Rows(0).Item("Order_Code") = txt_OrderCode.Text
            dsDelete.Tables(0).Rows(0).Item("Alternative_Code") = txt_AlternativeCode.Text
            ...
            If dgvShowData.GetSelectedIndex IsNot Nothing AndAlso dgvShowData.GetSelectedIndex.Count > 0 Then
                ...
                For i As Integer = 0 To dgvShowData.GetSelectedIndex.Count - 1
                    ...
                    dsDelete.Tables(0).Rows.Add()
                    dsDelete.Tables(0).Rows(count).Item("Order_Code") = drData.Item("Order_Code")
                    dsDelete.Tables(0).Rows(count).Item("Alternative_Code") = drData.Item("Alternative_Code")
                    count = count + 1
            ...
            '自資料庫刪除
            count = PUB.DeletePUBOrder(dsDelete)
...
#Region "     *欄位檢查 - True 檢查不通過;False 檢查通過 "
    ...
    Public Function fieldCheckResult(ByVal iButtonFlag As Integer) As Boolean
        ...
        Try
            ...
            '設定檢查的控制項、檢查未通過的顯示名稱
            Dim objCheck1 As UCLTextCodeQueryUI = txt_OrderCode
            Dim errorStr1 As String = "醫令代碼、"

            Dim objCheck2 As UCLTextCodeQueryUI = txt_AlternativeCode
            Dim errorStr2 As String = "可替代醫令代碼"
    ...
    Public Overrides Sub dgvCellClick()
        ...
        Try
            ...
            If dgvShowData.CurrentCellAddress.Y >= 0 Then
                ...
                '設定英文名稱的控制項
                txt_AlternativeCode.uclCodeValue = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Alternative_Code").Value)
                txt_AlternativeCode.ProcessQuery(txt_AlternativeCode.uclCodeValue.ToString)
...
#Region "     *取得要存檔的Dataset "
    ...
    Private Function GetSaveDS(ByVal inputCreateUser As String, ByVal inputModifiedUser As String) As DataSet
        ...
        Try
            ...
            '{ Station_No,Print_Time,Printer_Code,Create_User,Create_Time,Modified_User ,Modified_Time   }
            drSave("Order_Code") = nvl(txt_OrderCode.Text)

            drSave("Alternative_Code") = nvl(txt_AlternativeCode.Text)
