...
Public Class PUBAreaCodeNSetupUI
...
#Region "     *Table 欄位 "
    ...
    '存檔的Table欄位
    Private gblColumnNameDB As String() = PubAreaCodeNDataTableFactory.columnsName

    '存檔的Table Name
    Private gblTableName As String = PubAreaCodeNDataTableFactory.tableName

    '空的DT，清空資料使用
    Private gblEmptyDT As DataTable = PubAreaCodeNDataTableFactory.getDataTable

    '要顯示在畫面上的Grid 中文標頭，不顯示的也要有名稱
    Private gblColumnTextDgv As String = "戶籍地代碼,原戶籍地代碼,戶籍地名稱,縣市代碼,排序"

    '要顯示在畫面上的Grid 欄位寬度，不顯示的設0
    Private gblColumnWidthDgv As String = "100,100,200,100,100"
...
#Region "     *新增 "
    ...
    Public Overrides Function InsertData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.PUBAreaCodeNInsertByPK(InsertDS)
            ...
            If count > 0 Then
                ...
                updateDataGridView(iButtonFlag, PUB.PUBAreaCodeNQueryByPKROC(txt_AreaCode.Text))
...
#Region "     *修改 "
    ...
    Public Overrides Function updateData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = PUB.PUBAreaCodeNUpdateByPK(updateDS)
...
#Region "     *刪除 "
    ...
    Public Overrides Function deleteData() As Boolean
        ...
        Try
            ...
            dsDelete.Tables.Add()
            dsDelete.Tables(0).Columns.Add("Area_Code")
            dsDelete.Tables(0).Columns.Add("Orig_Area_Code")

            dsDelete.Tables(0).Rows.Add()
            dsDelete.Tables(0).Rows(0).Item("Area_Code") = txt_AreaCode.Text
            dsDelete.Tables(0).Rows(0).Item("Orig_Area_Code") = txt_OrigAreaCode.Text
            ...
            '自資料庫刪除
            count = PUB.PUBAreaCodeNdeleteChoose(dsDelete)
...
#Region "     *查詢 "
    ...
    Public Overrides Function queryData() As Boolean
        ...
        Try
            ...
            If txt_AreaCode.Text <> "" Then
                queryDS = PUB.PUBAreaCodeNQueryByPKROC(txt_AreaCode.Text)
            Else
                queryDS = PUB.queryAreaCodeAll()
            End If
            '*******************************************************************************
...
#Region "     *欄位檢查 - True 檢查不通過;False 檢查通過 "
    ...
    Public Function fieldCheckResult(ByVal iButtonFlag As Integer) As Boolean
        ...
        Try
            ...
            '設定檢查的控制項、檢查未通過的顯示名稱
            Dim objCheck1 As UCLTextBoxUC = txt_AreaCode
            Dim errorStr1 As String = "戶籍地"

            Dim objCheck2 As UCLTextBoxUC = txt_OrigAreaCode
            Dim errorStr2 As String = "原戶籍地"
    ...
    Public Overrides Sub dgvCellClick()
        ...
        Try
            ...
            If dgvShowData.CurrentCellAddress.Y >= 0 Then
                ...
                '設定戶籍地代碼的控制項
                txt_AreaCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Area_Code").Value)

                '設定原戶籍地代碼的控制項
                txt_OrigAreaCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Orig_Area_Code").Value)

                '設定戶籍地名稱的控制項
                txt_AreaName.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Area_Name").Value)

                '設定縣市代碼的控制項
                txt_CountyCode.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("County_Code").Value)

                '設定排序的控制項
                txt_SortValue.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Sort_Value").Value)
...
#Region "     *取得要存檔的Dataset "
    ...
    Private Function GetSaveDS(ByVal inputCreateUser As String, ByVal inputModifiedUser As String) As DataSet
        ...
        Try
            ...
            '{ Station_No,Print_Time,Printer_Code,Create_User,Create_Time,Modified_User ,Modified_Time   }
            drSave("Area_Code") = nvl(txt_AreaCode.Text)

            drSave("Orig_Area_Code") = nvl(txt_OrigAreaCode.Text)

            drSave("Area_Name") = nvl(txt_AreaName.Text)

            drSave("County_Code") = nvl(txt_CountyCode.Text)

            drSave("Sort_Value") = nvl(txt_SortValue.Text)
