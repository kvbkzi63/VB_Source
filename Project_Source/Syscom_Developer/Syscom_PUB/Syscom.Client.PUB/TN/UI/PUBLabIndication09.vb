...
Public Class PUBLabIndication09
    Inherits BaseFormUI
...
#Region "     使用的Instance宣告 "
    ...
    Private Shared myInstance As PUBLabIndication09
    Public Shared Function GetInstance() As PUBLabIndication09
        If myInstance Is Nothing Then
            myInstance = New PUBLabIndication09
        End If
        Return myInstance
    End Function
...
#Region "     儲存 "
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            ...
            'If errorCode Then
            '    returnBoolean = False
            '    MessageHandling.ShowWarnMsg("CMMCMMB300", "部位不可為空白，請輸入！")
    ...
    Public Sub ShownMe() Handles Me.Shown
        ...
        Try
            ...
            If gblXmlData <> "" Then
                ...
                If pvtData.Tables("部位一").Rows(0).Item("部位一").ToString.Trim <> "" Then
                    txt_BodySite1.Text = pvtData.Tables("部位一").Rows(0).Item("部位一").ToString.Trim
                End If

                If pvtData.Tables("部位二").Rows(0).Item("部位二").ToString.Trim <> "" Then
                    txt_BodySite2.Text = pvtData.Tables("部位二").Rows(0).Item("部位二").ToString.Trim
                End If

                If pvtData.Tables("部位三").Rows(0).Item("部位三").ToString.Trim <> "" Then
                    txt_BodySite3.Text = pvtData.Tables("部位三").Rows(0).Item("部位三").ToString.Trim
                End If
            ...
            Select Case gblTqty
                Case 1
                    txt_BodySite1.Enabled = True
                    txt_BodySite2.Enabled = False
                    txt_BodySite3.Enabled = False

                Case 2
                    txt_BodySite1.Enabled = True
                    txt_BodySite2.Enabled = True
                    txt_BodySite3.Enabled = False

                Case 3
                    txt_BodySite1.Enabled = True
                    txt_BodySite2.Enabled = True
                    txt_BodySite3.Enabled = True

                Case Else
                    txt_BodySite1.Enabled = False
                    txt_BodySite2.Enabled = False
                    txt_BodySite3.Enabled = False
...
#Region "     事件集合 "
    ...
    Private Function ChkselectRbtRatio() As Boolean
        ...
        Select Case gblTqty
            Case 1
                If nvl(txt_BodySite1.Text) = "" Then
                    Return True
                End If
            ...
            Case 2
                ...
                If nvl(txt_BodySite2.Text) = "" Then
                    Return True
                End If
            ...
            Case 3
                ...
                If nvl(txt_BodySite3.Text) = "" Then
                    Return True
                End If
...
#Region "     取得勾選的Dataset資料 "
    ...
    Private Function getCheckDataByDataSet() As DataSet
        ...
        rowDt1.Item("特殊屬性表單代碼") = "PUBLabIndication09"
        rowDt1.Item("特殊屬性表單名稱") = "細菌檢驗申請單"
        dsCheckData.Tables(0).Rows.Add(rowDt1)
        '--------------------------------------------------------------------------------
        'Dataset:建第2個table
        dsCheckData.Tables.Add()
        dsCheckData.Tables(1).TableName = "部位"
        dsCheckData.Tables(1).Columns.Add("部位")

        '存值入各欄位
        Dim rowDt2 As DataRow = dsCheckData.Tables(1).NewRow
        rowDt2.Item("部位") = nvl(txt_BodySite1.Text)
        dsCheckData.Tables(1).Rows.Add(rowDt2)

        '--------------------------------------------------------------------------------
        'Dataset:建第3個table
        dsCheckData.Tables.Add()
        dsCheckData.Tables(2).TableName = "部位"
        dsCheckData.Tables(2).Columns.Add("部位")

        '存值入各欄位
        Dim rowDt3 As DataRow = dsCheckData.Tables(2).NewRow
        rowDt3.Item("部位") = nvl(txt_BodySite2.Text)
        dsCheckData.Tables(2).Rows.Add(rowDt3)

        '--------------------------------------------------------------------------------
        'Dataset:建第4個table
        dsCheckData.Tables.Add()
        dsCheckData.Tables(3).TableName = "部位"
        dsCheckData.Tables(3).Columns.Add("部位")

        '存值入各欄位
        Dim rowDt4 As DataRow = dsCheckData.Tables(3).NewRow
        rowDt4.Item("部位") = nvl(txt_BodySite3.Text)
        ...
        dsCheckData.Tables(5).Columns.Add("BodySite1")
        dsCheckData.Tables(5).Columns.Add("BodySite2")
        dsCheckData.Tables(5).Columns.Add("BodySite3")

        '存值入各欄位
        Dim rowDt6 As DataRow = dsCheckData.Tables(5).NewRow
        rowDt6.Item("Indication_Type") = "2" '1:By檢驗單 2:By Order
        rowDt6.Item("BodySite1") = nvl(txt_BodySite1.Text)
        rowDt6.Item("BodySite2") = nvl(txt_BodySite2.Text)
        rowDt6.Item("BodySite3") = nvl(txt_BodySite3.Text)
    ...
    Private Function getCheckDataByArray() As String()
        ...
        Dim pvtResult(1) As String

        pvtResult(0) = "2"  '1:By檢驗單 2:By Order

        Select Case gblTqty
            Case 1
                pvtResult(1) = "部位：" & txt_BodySite1.Text.Trim

            Case 2
                pvtResult(1) = "部位：" & txt_BodySite1.Text.Trim
                pvtResult(1) &= "<@@>部位：" & txt_BodySite2.Text.Trim

            Case 3
                pvtResult(1) = "部位：" & txt_BodySite1.Text.Trim
                pvtResult(1) &= "<@@>部位：" & txt_BodySite2.Text.Trim
                pvtResult(1) &= "<@@>部位：" & txt_BodySite3.Text.Trim
