...
#Region "     Table 欄位 "
    ...
    '存檔的Table欄位
    Private gblColumnNameDB As String() = PubReportAlarmDataTableFactory.columnsName

    '存檔的Table Name
    Private gblTableName As String = PubReportAlarmDataTableFactory.tableName

    '空的DT，清空資料使用
    Private gblEmptyDT As DataTable = PubReportAlarmDataTableFactory.getDataTable

    '要顯示在畫面上的Grid 中文標頭，不顯示的也要有名稱
    Private gblColumnTextDgv As String = "報表ID,報表名稱,警示筆數設定,是否生效啟用"

    '要顯示在畫面上的Grid 欄位寬度，不顯示的設0
    Private gblColumnWidthDgv As String = "150,150,130,130"
...
#Region "     新增 "
    ...
    ''' <remarks>by Hsiao.Kaiwen on 2015-8-7</remarks>
    Public Overrides Function InsertData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = Pub.PUBReportAlarminsert(InsertDS)
            ...
            If count > 0 Then
                ...
                Dim RptIsActive As String = ""
                If rbt_RptIsActiveYes.Checked Then
                    RptIsActive = "Y"
                ElseIf rbt_RptIsActiveNo.Checked Then
                    RptIsActive = "N"
                End If
                updateDataGridView(iButtonFlag, Pub.PUBReportAlarmQueryByLikeColumn(nvl(txt_ReportID.Text), nvl(txt_ReportName.Text), nvl(txt_RptAlarmCount.Text), nvl(RptIsActive)))
...
#Region "     修改 "
    ...
    Public Overrides Function updateData() As Boolean
        ...
        Try
            ...
            '寫入資料庫
            Dim count As Integer = Pub.PUBReportAlarmupdate(updateDS)
...
#Region "     刪除 "
    ...
    Public Overrides Function deleteData() As Boolean
        ...
        Try
            ...
            '自資料庫刪除
            Dim count As Integer = Pub.PUBReportAlarmdelete(nvl(txt_ReportID.Text))
    ...
    Public Overrides Function queryData() As Boolean
        ...
        Try
            ...
            ElseIf rbt_RptIsActiveNo.Checked Then
                RptIsActive = "N"
            End If
            queryDS = Pub.PUBReportAlarmQueryByLikeColumn(nvl(txt_ReportID.Text), nvl(txt_ReportName.Text), nvl(txt_RptAlarmCount.Text), nvl(RptIsActive))
...
#Region "     事件 "
    ...
    Private Sub cmb_ReportID_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_ReportID.SelectedIndexChanged

        txt_ReportID.Text = cmb_ReportID.SelectedValue
...
#Region "    初始化 - Combobox "
    ...
    ''' 初始化 - ComboBox
    ''' </summary>
    ''' <remarks>by Hsiao.Kaiwen on 2015-9-15</remarks>
    Private Sub initialComboBox()

        Dim dsDB As DataSet
        '執行查詢()
        dsDB = Pub.PUBReportAlarmQueryReportId()

        Try

            If CheckHasValue(dsDB) Then
                Me.cmb_ReportID.DataSource = dsDB.Tables(0)
...
#Region "     欄位檢查 - True 檢查不通過;False 檢查通過 "
    ...
    Public Function fieldCheckResult(ByVal iButtonFlag As Integer) As Boolean
        ...
        Dim stbErrorCollectionValue As StringBuilder = New StringBuilder
        ...
        Try
            ...
            '設定檢查的控制項、檢查未通過的顯示名稱
            Dim objCheck1 As UCLTextBoxUC = txt_ReportID
            Dim errorStr1 As String = "報表ID、"
            ...
            Select Case iButtonFlag
                ...
                Case buttonAction.INSERT, buttonAction.UPDATE
                    ...
                    If IsNumeric(txt_RptAlarmCount.Text) = True Then
                        If CInt(txt_RptAlarmCount.Text) < 100 Then
                            stbErrorCollectionValue.Append("警示筆數必須大於100、")
                            returnBoolean = True
                        End If
                    End If
            ...
            If returnBoolean Then
                ...
                Dim finalErrorStringValue As String = stbErrorCollectionValue.ToString

                '有值
                If finalErrorString.Length > 0 And finalErrorStringValue.Length = 0 Then
                ...
                ElseIf finalErrorString.Length = 0 And finalErrorStringValue.Length > 0 Then

                    '去掉最後一個逗號
                    finalErrorStringValue = finalErrorStringValue.Substring(0, finalErrorStringValue.Length - 1)

                    MessageHandling.ShowErrorMsg("CMMCMMB300", finalErrorStringValue)
                ...
                ElseIf finalErrorString.Length > 0 And finalErrorStringValue.Length > 0 Then
                    ...
                    finalErrorString = finalErrorString & "不得為空、" & finalErrorStringValue
    ...
    Public Overrides Sub dgvCellClick()
        ...
        Try
            ...
            If dgvShowData.CurrentCellAddress.Y >= 0 Then
                ...
                '設定警示筆數設定的控制項
                txt_RptAlarmCount.Text = StringUtil.nvl(dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Rpt_Alarm_Count").Value)

                '設定是否生效啟用的控制項

                If dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Rpt_Is_Active").Value = "Y" Then

                    rbt_RptIsActiveYes.Checked = True

                ElseIf dgvShowData.Rows(dgvShowData.CurrentCellAddress.Y).Cells("Rpt_Is_Active").Value = "N" Then
                    rbt_RptIsActiveNo.Checked = True
                End If
...
#Region "     取得要存檔的Dataset "
    ...
    Private Function GetSaveDS(ByVal inputCreateUser As String, ByVal inputModifiedUser As String) As DataSet
        ...
        Try
            ...
            drSave("Report_ID") = nvl(txt_ReportID.Text.ToString.Trim)

            drSave("Rpt_Alarm_Count") = nvl(txt_RptAlarmCount.Text.ToString.Trim)
            drSave("Rpt_Count") = DBNull.Value
            Dim RptIsActive As String = ""
            If rbt_RptIsActiveYes.Checked Then
                RptIsActive = "Y"
            ElseIf rbt_RptIsActiveNo.Checked Then
            ...
            drSave("Rpt_Is_Active") = nvl(RptIsActive)
