...
#Region "     使用者宣告　"
    ...
    '取得維護表格名稱
    Dim DBName As String = PubSyscodeTypeDataTableFactory.tableName

    'Grid使用的標題--"Status"與"ErrMsg","Check_Index"為新增隱藏字段
    Dim columnNameGrid() As String = {"類別代碼", "類別名稱", "類別代碼明細", "建立者", "建立時間", "修改者", "修改時間", "Status", "ErrMsg", "Check_Index"}

    '設定Grid欄位寬度
    Dim columnWidth As String = "100,300,120,80,160,80,160"

    '設定Grid顯示欄位
    Dim columnVisible As String = "0,1,2,3,4,5,6,7"

    '設定可編輯的欄位位置索引
    Dim columnEditIndex() As String = {"1", "2", "3"}

    '設定必填欄位位置索引
    Dim columnNotNullIndex() As String = {"1", "2"}
    ...
    '取得維護表字段名
    Dim columnNameDB() As String = {"Type_Id", "Type_Name", "Btn_Detail", "Create_User", "Create_Time", "Modified_User", "Modified_Time"}

    '取得維護表字段長度
    Dim columnsLength() As Integer = {-1, 50, -1, 10, -1, 10, -1}
    '資料集類型定義
    Enum DataSet_Type
        Grid = 0
        DB = 1
        ...
        Try
            ...
            If checkFieldIsNull() Then
                ...
                If dsSave IsNot Nothing AndAlso dsSave.Tables IsNot Nothing AndAlso dsSave.Tables(0).Rows.Count > 0 Then
                    dsResult = Pub.savePUBSyscodeType(dsSave)
        ...
        Try
            ...
            If dsDelete IsNot Nothing AndAlso dsDelete.Tables IsNot Nothing AndAlso dsDelete.Tables(0).Rows.Count > 0 Then
                ...
                If (MessageHandling.ShowQuestionMsg("CMMCMMB932", New String() {}) = Windows.Forms.DialogResult.Yes) Then
                    ...
                    '執行刪除
                    dsResult = Pub.deletePUBSyscodeType(dsDelete)
...
#Region "     查詢 "
    ...
    Private Function QueryData() As Boolean
        ...
        Try
            ...
            '    dsDB = Pub.qureyPUBSyscodeTypeAll

            'Else
            dsDB = Pub.queryPUBSyscodeTypeLikePKey(txt_Type_Id.Text.Trim)
...
#Region "     初始化 - DataGridView "
    ...
    Private Sub ShowDgv(ByVal inDsData As DataSet)
        ...
        Try
            ...
            Dim txt_cell0 As New TextBoxCell()
            ...
            htGrid.Add(3, btn_cell1)
            htGrid.Add(4, txt_cell0)
            htGrid.Add(5, txt_cell0)
            htGrid.Add(6, txt_cell0)
            htGrid.Add(7, txt_cell0)
...
#Region "Grid相關處理"
    ...
    Private Sub dgv_Data_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_Data.CellClick
        If e.ColumnIndex = 3 Then
            Dim openDlg As PubSyscodeUI = New PubSyscodeUI(dgv_Data.Rows(e.RowIndex).Cells("類別代碼").Value,
                                                           dgv_Data.Rows(e.RowIndex).Cells("類別名稱").Value)
            openDlg.ShowDialog()
        End If
    End Sub
    ...
    Private Function getCheckData() As DataSet
        ...
        For i = 0 To dgv_Data.Rows.Count - 1
            If dgv_Data.Rows(i).Cells(0).Value Then
                ...
                drCheckData("Type_Id") = dgv_Data.Rows(i).Cells("類別代碼").Value.ToString.Trim
                drCheckData("Type_Name") = dgv_Data.Rows(i).Cells("類別名稱").Value.ToString.Trim
