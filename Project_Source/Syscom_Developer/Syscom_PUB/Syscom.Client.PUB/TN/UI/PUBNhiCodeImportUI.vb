...
'*        Description:	轉入健保醫令價格異動檔
'*          Copyright:	Copyright(c) Syscom Computer Co,.Ltd
'*            Company:	Syscom Computer Co,.Ltd
'*            @author:	Chi,Wang
'*        Create Date:	2016-09-19
'*      Last Modifier:	Chi,Wang
'*   Last Modify Date:	2016-09-19
'*
'*****************************************************************************
'*/
...
Public Class PUBNhiCodeImportUI
...
#Region "     使用的Instance宣告 "
    ...
    Private Shared myInstance As PUBNhiCodeImportUI
    Public Shared Function GetInstance() As PUBNhiCodeImportUI
        If myInstance Is Nothing Then
            myInstance = New PUBNhiCodeImportUI
        End If
        Return myInstance
    End Function
...
#Region "     匯入excel按鈕 "
    ...
    '匯入excel按鈕  宣告方法
    Private Sub ImportExcel(ByVal ImportDs As DataSet) Handles pvtReceiveMgr.ImportExcel
        Try
            pvtReceiveMgr.RaiseUclWaitingForm("WaitingStart", "確定中，請稍後‧‧‧")
            Dim Sourceds As DataSet = ImportDs
            Dim Import As DataSet = ImportDs.Clone
            For i As Integer = 0 To ImportDs.Tables(0).Rows.Count - 1
                If ImportDs.Tables(0).Rows(i).Item("項次").ToString.Trim <> "" Then
                    Import.Tables(0).ImportRow(ImportDs.Tables(0).Rows(i))
                End If
            Next
            If txt_version.Text <> "" Then
                '匯入excel 資料
                pub.PUBNhiCodeImportBOE1ImportCase(Import, txt_version.Text.Trim, CurrentUserID)
            End If
...
#Region "     查詢 "
    ...
    Private Function QueryData() As Boolean
        ...
        Try
            ...
            Dim queryds As DataSet = pub.PUBNhiCodeImportBOE1query(txt_version.Text.Trim, txt_insucode.Text.Trim, Ucldtp_effectdate.GetUsDateStr)
...
#Region "     轉入醫令價格檔 "
    ...
    Private Function TransforData(ByVal ds As DataSet) As Boolean
        ...
        Try
            ...
            Dim returnBoolean As Boolean = False
            Dim count As Integer = pub.PUBNhiCodeImportBOE1importorderprice(ds, CurrentUserID)
            If count > 0 Then
                returnBoolean = True
            End If
            Return returnBoolean
...
#Region "     初始化 - 顯示UI "
    ...
    Public Sub ShownMe() Handles Me.Shown
        ...
        Try
            ...
            UclExcelImportUC1.Enabled = False
        Catch cmex As CommonException
            Throw cmex
        Catch ex As Exception
...
#Region "txt_version.Text不可為空"
    Public Sub check() Handles txt_version.TextChanged

        If txt_version.Text <> "" Then
            UclExcelImportUC1.Enabled = True
        Else
            UclExcelImportUC1.Enabled = False
        End If
    End Sub
