...
Public Class PUBDiseaseBO_E1
    Inherits PubDiseaseBO

    'Private m_tableName = "PUB_Disease"

    'Public Shared ReadOnly Property Instance()
    '    Get
    '        Return New PUBDiseaseBO_E1
    '    End Get
    'End Property
...
#Region "########## getInstance ##########"
    ...
    Private Shared instance As PUBDiseaseBO_E1

    Public Overloads Shared Function getInstance() As PUBDiseaseBO_E1
        If instance Is Nothing Then
            instance = New PUBDiseaseBO_E1
        End If
        Return instance
    End Function
...
#End Region
    ...
    ''' <date>2009-07-06</date>
    ''' <remarks></remarks>
    Public Function queryPUBDiseaseByCond(ByVal _icdCode As String, ByVal _diseaseEnName As String, ByVal _diseaseTypeId As String) As DataSet
            ...
            Using conn As SqlConnection = SQLConnFactory.getInstance.getPubDBSqlConn
                Using Command As SqlCommand = conn.CreateCommand
                    Command.CommandText = "SELECT '' AS Favor_Category" & _
                                           ...
                                           " FROM PUB_Disease WITH(NOLOCK)" & _
                                          " WHERE Icd_Code LIKE @Icd_Code+'%' " & _
                                            ...
                                            " AND PUB_Disease.Disease_Type_Id = '1'" & _
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function updateDiseaseICDColumn(ByVal diseaseDateDT As DataTable, ByVal outDate As Date, Optional ByRef conn As System.Data.IDbConnection = Nothing) As Integer
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function cancelDisease(ByVal cancelDT As DataTable, ByVal EndDate As Date) As Integer
        ...
        If DataSetUtil.IsContainsData(cancelDT) Then
            ...
            'update SQL
            '----------------------------------------------------------------------------
            cmdStr.Append("update ").Append(tableName).Append(" set DC = @DCY ")
            '----------------------------------------------------------------------------
            '----------------------------------------------------------------------------
            'Where SQL
            '----------------------------------------------------------------------------
    ...
    ''' 1.2009/09/16, XXX
    ''' </修改註記>
    Public Function countDisease(ByVal ConditionDT As DataTable) As Integer
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Function deleteDisease(ByVal EffectDate As Date, ByVal IcdCode As String) As Integer
        Try
            Dim count As Integer = 0
    ...
    ''' 1.2009/12/22, XXX
    ''' </修改註記>
    Public Function queryExistIcdCodeByCondition(ByVal Condition As String) As DataTable
    ...
    ''' 1.2010/03/01, XXX
    ''' </修改註記>
    Public Function queryPreOrNextRecordSingleDisease(ByVal partialIcdCode As String, ByVal diseaseTypeId As String, ByVal isPre As Boolean) As DataTable
    ...
    Public Function getPsyIcdCodeData(ByVal opdVisitDate As Date, ByVal icdCode As String) As String
        ...
        Try
            cmdStr.AppendLine("select distinct rtrim(Icd_Code) from PUB_Disease")
