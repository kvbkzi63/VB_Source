...
#Region "阻止外部直接宣告"
    ...
    Public Sub Info(ByRef RptParam As Object())
        ...
        Try
            Dim ds As DataSet = pubService.getReportInfo(RptParam(0), RptParam(1), RptParam(2))
            reportId = RptParam(0)
            ...
            If ds.Tables("PUB_Hospital").Rows.Count > 0 Then
                With ds.Tables("PUB_Hospital").Rows(0)
                    ...
                    _Principal_Name = .Item("Principal_Name").ToString '無資料請建檔
                    _Principal_Email = .Item("Principal_Email").ToString '無資料請建檔
                    _Hospital_Level_Id = .Item("Hospital_Level_Id").ToString '無資料請建檔
                    _URL = .Item("URL")
                    _Unified_Business_No = .Item("Unified_Business_No").ToString '無資料請建檔
