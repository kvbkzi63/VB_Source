...
'*****************************************************************************
'*
'*    Page/Class Name:  子系統維護(多筆)
'*              Title:	SubSystemMtnUIMult
'*        Description:	
...
Public Class SubSystemMtnUIMult
...
#Region "     使用者宣告　"
    ...
    '取得維護表格名稱
    Private DBName As String = ArmSubSystemDataTableFactory.tableName

    'Grid使用的標題
    Private columnNameGrid() As String = {"系統代碼", "系統名稱", "子系統代碼", "子系統名稱", "排列順序", "是否有效", "是否有效中文", "建立者", "建立時間", "修改者", "修改時間"}

    '設定Grid欄位寬度
    Private columnWidth As String = "100,150,100,150,100,100,100,100,150,100,150"

    '設定Grid顯示欄位
    Private columnVisible As String = "1,2,3,4,5,6,8,9,10,11"

    '設定可編輯的欄位位置索引
    Private columnEditIndex() As String = {"3", "4", "5", "6"}

    '設定必填欄位位置索引
    Private columnNotNullIndex() As String = {"3", "4", "5", "6"}

    '設定PKey欄位位置索引
    Private columnPKeyIndex() As String = {"1"}

    '設定建立時間與修改時間的欄位索引
    Private gblCreateTimeIdx As Integer = 9
    Private gblModifiedTimeIdx As Integer = 11

    '是否的 Code Type 代號
    Private CodeTypeDC As String = "9000"

    '取得維護表字段名
    Private columnNameDB() As String = ArmSubSystemDataTableFactory.columnsName

    '取得維護表字段長度
    Private columnsLength() As Integer = ArmSubSystemDataTableFactory.columnsLength
            ...
            If checkFieldIsNull() Then
                ...
                If dsSave IsNot Nothing AndAlso dsSave.Tables IsNot Nothing AndAlso dsSave.Tables(0).Rows.Count > 0 Then
                    ...
                    For Each row As DataRow In dsSave.Tables(0).Rows
                        count += ARM.SubDelete(row("org_sub_system_no"))
                    Next
                    count += ARM.SubInsert(dsSave)
                    ...
                    Else
                        '若成功則將畫面資料修改
                        Dim dsDB As DataSet = ARM.SubGetSubSystem("", "", cbo_AppSystem.SelectedValue, "", "")
                        ShowDgv(dsDB)
                    End If
                Else
            ...
            If dsDelete IsNot Nothing AndAlso dsDelete.Tables IsNot Nothing AndAlso dsDelete.Tables(0).Rows.Count > 0 Then
                ...
                If (MessageHandling.ShowQuestionMsg("CMMCMMB932", New String() {}) = Windows.Forms.DialogResult.Yes) Then
                    ...
                    For Each row As DataRow In dsDelete.Tables(0).Rows
                        count += ARM.SubDelete(row("sub_system_no"))
                    Next
...
#Region "     初始化 - DataGridView "
    ...
    Private Sub ShowDgv(ByVal inDsData As DataSet)
        Try
            ...
            cbo_cell3.Ds = Cmm.CMMSysCodeBSSysCodeQuery(CodeTypeDC)
            cbo_cell3.DisplayIndex = "1"
            ...
            '增加原始系統代碼欄位
            If Not dsData.Tables(0).Columns.Contains("原子系統代碼") Then
                dsData.Tables(0).Columns.Add("原子系統代碼")
            End If

            For Each row As DataRow In dsData.Tables(0).Rows
                row("原子系統代碼") = row("子系統代碼")
            Next
            ...
            htGrid.Add(3, txt_cell1)
            htGrid.Add(4, txt_cell2)
            htGrid.Add(5, txt_cell3)
            htGrid.Add(6, cbo_cell3)
...
#Region "     取得勾選的Dataset資料 "
    ...
    Private Function getCheckData() As DataSet
        Try
            ...
            dsCheckData.Tables(0).Columns.Add("org_sub_system_no")
            ...
            For i = 0 To dgv_Data.Rows.Count - 1
                With dgv_Data.GetDBDS.Tables(0).Rows(i)
                    If dgv_Data.Rows(i).Cells(0).Value Then
                        ...
                        drCheckData("sub_app_system_no") = .Item("系統代碼").ToString.Trim
                        drCheckData("sub_system_no") = .Item("子系統代碼").ToString.Trim
                        drCheckData("sub_system_name") = .Item("子系統名稱").ToString.Trim
                        drCheckData("sub_display_order") = .Item("排列順序").ToString.Trim
                        drCheckData("sub_flag_no") = .Item("是否有效").ToString.Trim
                        drCheckData("sub_creator_no") = CurrentUserID
                        drCheckData("sub_create_datetime") = pvtSysTime
                        drCheckData("sub_operator_no") = CurrentUserID
                        drCheckData("sub_update_datetime") = pvtSysTime
                        drCheckData("org_sub_system_no") = .Item("原子系統代碼").ToString.Trim
